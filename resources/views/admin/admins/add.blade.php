...
@section('main_content')  
    ...
    <section class="content">
      ...
      <div class="box box-success">
        ...
        <!-- /.box-header -->
        <!-- form start -->
        <form role="form" id="ajax_form" action="{{ route('admins.store') }}" method="POST" enctype="multipart/form-data">
          <div class="box-body">
            {{ csrf_field() }}

            <div class="form-group">
              <label for="admin_name">Admin Name</label>
              <input type="text" name="name" class="form-control" id="admin_name" placeholder="Enter Admin Name" value="{{ old('name') }}">
            </div>
            ...
            <div class="form-group">
              ...
              @foreach ($roles as $role)
                <label><input type="checkbox" name="role[]" class="flat-red" value="{{ $role->id }}">  {{ $role->name }}  </label>
              @endforeach
            </div>

            <div class="form-group">
              <label for="">Status</label> <br>
                <label><input type="checkbox" name="status" checked="" class="flat-red" value="1">  Active  </label>
            </div>
