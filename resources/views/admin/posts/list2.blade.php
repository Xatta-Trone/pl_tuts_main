           ...
           <table id="example1" class="table table-bordered table-striped table-responsive">
             ...
             @foreach($posts as $post)
                <tr>
                  <td>{{ $loop->index +1 }}</td>
                  <td>{{ $post->name }}</td>
                  <td>{{ $post->department_slug }}</td>
                  <td>{{ $post->level_term_slug }}</td>
                  <td>{!! $post->courseData() !!}</td>
                  <td>{{ $post->uploader()->name }}</td>
                  <td> {!! $post->status($post->id)  !!} </td>
                  <td>
                      <a href="#" class="btn btn-primary" id="modal_switch" data-id={{ $post->id }}><i class="fa fa-eye"></i></a>
                    @can('post_update')
                      <a href="{{ route('posts.edit',$post->id) }}" class="btn btn-primary"><i class="fa fa-pencil"></i></a>
                    @endcan
                    @can('post_delete')
                      <a href="#" onclick="if(confirm('are you sure ?')){ event.preventDefault(); document.getElementById('delete-form-{{$post->id}}').submit();}else{event.preventDefault();}" class="btn btn-danger"><i class="fa fa-trash-o"></i></a>
